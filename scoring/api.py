...
class Field(abc.ABC):
    ...
    def __get__(self, obj, objtype=None):
        return getattr(obj, self.private_name, None) if self.nullable else getattr(obj, self.private_name)
    ...
    def validate(self, value):
        if isinstance(value, str):
            if len(value) != 11 or not re.match(r"^7\d{10}", value):
                raise ValueError(f'Expected {value!r} to be a 11-digit string starting with 7')
    ...
    def validate(self):
        ...
        for valid_combination in self._valid_fields_combinations:
            if len(set(valid_combination) - presented_fields) == 0:
                ...
                break
        if invalid_request:
            raise ValueError(f"Expected one if required fields set: {self._valid_fields_combinations} ")
...
def make_request(body: dict, target_class: type):
    # req = target_class()
...
def interests_handler(ctx, store, body, is_admin):
    clients_interest_req: ClientsInterestsRequest = make_request(body, ClientsInterestsRequest)
...
def method_handler(request, ctx, store):
    ...
    try:
        ...
        if check_auth(request=method_request):  # make authentication
            ...
            try:
                response = router[method_request.method](ctx, store,
                ...
                code = BAD_REQUEST
                response = "Expected method to be one of [online_score, clients_interests]"
        else:
            code = FORBIDDEN
